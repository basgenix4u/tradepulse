...
echo "=== Building TradePulse v2.0 ==="

# Backend structure
mkdir -p backend/cmd/server
mkdir -p backend/internal/{auth,config,db,handlers,middleware,models,websocket}

# Create all backend files
cat > backend/cmd/server/main.go << 'ENDGO'
package main
import (
	"log"
	"os"
